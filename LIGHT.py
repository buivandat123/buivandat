...
class CommandHandler:
    def __init__(self, client):
        self.client = client
        self.LIGHT = ThreadSafeDict(self._load_modules(MODULES_DIR, 'LIGHT', ['version', 'credits', 'description', 'power']))
        self.noprefix_LIGHT = ThreadSafeDict(self._load_modules(NOPREFIX_MODULES_DIR, 'LIGHT', ['version', 'credits', 'description']))
        self.auto_LIGHT = ThreadSafeDict(self._load_auto_modules())
    ...
    def _create_prefix_handlers(self):
        return sorted([(self.current_prefix + command, handler)
                       for command, handler in self.LIGHT.items() if self.current_prefix],
    ...
    def predict_command(self, wrong_command):
        possible_commands = list(self.LIGHT.keys())
    ...
    def _handle_cooldown(self, message, message_object, thread_id, thread_type, author_id):
        ...
        if action == 'set':
            ...
            is_valid_command = command in self.LIGHT or command in self.stw_commands
    ...
    def handle_command(self, message, author_id, message_object, thread_id, thread_type):
        ...
        noprefix_handler = self.noprefix_LIGHT.get(message_text.lower())
        if noprefix_handler:
        ...
        if message_text.startswith(self.current_prefix):
            ...
            if command_part_after_prefix:
                ...
                if potential_command_name_base in self.LIGHT:
                    found_cmd_name = potential_command_name_base
                    command_type_reaction = 'normal'

        if not found_cmd_name and self.stw_commands:
            sorted_valid_stw_cmds = sorted([cmd for cmd in self.stw_commands if cmd in self.LIGHT], key=len, reverse=True)
        ...
        if found_cmd_name:
            ...
            command_handler = self.LIGHT.get(found_cmd_name)
            if command_handler:
