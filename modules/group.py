...
des = {"version":"1.0.0","credits":"kryzis X TXA","description":"Quản lý danh sách nhóm","power":"Admin"}

def _all_groups(client):
    try:
        gd=client.fetchAllGroups()
        res=[]
        if gd and hasattr(gd,"gridVerMap"):
            for gid in gd.gridVerMap:
                try:
                    name=client.fetchGroupInfo(gid).gridInfoMap.get(str(gid),{}).get("name","?")
                    res.append({"id":gid,"name":name})
                except: res.append({"id":gid,"name":"?"})
        return res
    except: return []

def _leave(client, gid):
    try:
        p={"grids":[str(gid)],"imei":client._imei,"silent":1,"language":"vi"}
        params={"params":client._encode(p),"zpw_ver":647,"zpw_type":30}
        r=client._get("https://tt-group-wpa.chat.zalo.me/api/group/leave",params=params)
        return r.json().get("error_code")==0
    except: return False

def handle_group(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if action=="show":
        groups=_all_groups(client)
        if not groups:
            _reply(client,message_object,thread_id,thread_type,"WARNING\n    Không có nhóm nào!",sty_warn); return
        lines=[f"  {i}. {g['name']}" for i,g in enumerate(groups,1)]
        _mention_send(client, author_id, thread_id, thread_type, f"NHÓM CÓ BOT ({len(groups)}) 📋", lines, "#15A85F")

    elif action=="leave" and len(parts)>2:
        groups=_all_groups(client)
        nums=[int(x) for x in parts[2].split(",") if x.isdigit()]
        ok=0
        for n in nums:
            if 1<=n<=len(groups) and str(groups[n-1]["id"])!=str(thread_id):
                if _leave(client,groups[n-1]["id"]): ok+=1
        _mention_send(client, author_id, thread_id, thread_type, "SUCCESS",
                      [f"✅ Đã rời {ok}/{len(nums)} nhóm!"], sty_ok if ok>0 else sty_warn)
    else:
