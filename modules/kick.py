...
des = {"version":"1.0.0","credits":"kryzis X TXA","description":"Đá thành viên khỏi nhóm","power":"Admin"}

def handle_kick(message, message_object, thread_id, thread_type, author_id, client):
    ...
    uids = ([m["uid"] for m in message_object.mentions] if message_object.mentions
            else [p for p in message.split()[1:] if p.isdigit()])
    if not uids:
        _reply(client,message_object,thread_id,thread_type,
               f"WARNING\n    {PREFIX}kick @user\n    {PREFIX}kick <uid>",sty_warn); return

    try:
        bot = str(client.uid)
    except: bot=None
    if bot and bot in [str(u) for u in uids]:
        _reply(client,message_object,thread_id,thread_type,"ERROR\n    Không thể đá bot!",sty_err); return

    try:
        client.kickUsersInGroup(uids, thread_id)
        names = " + ".join(_name(client,u) for u in uids)
        _mention_send(client, author_id, thread_id, thread_type, "SUCCESS",
                      [f"✅ Đã đá: {names}"], "#15A85F")
