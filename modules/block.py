...
des = {"version":"1.0.0","credits":"kryzis X TXA","description":"Chặn người dùng","power":"Admin"}
...
def handle_block(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if not uid:
        _reply(client,message_object,thread_id,thread_type,
               f"WARNING\n    {PREFIX}block @user",sty_warn); return

    try:
        client.blockUser(uid)
        _mention_send(client, uid, thread_id, thread_type, "SUCCESS",
                      ["🚫 Đã bị chặn bởi bot!"], "#15A85F")
