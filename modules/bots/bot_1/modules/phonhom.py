...
def handle_phonhom(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(parts)<2:
        _reply(client,message_object,thread_id,thread_type,
               f"WARNING\n    {PREFIX}phonhom @user\n    {PREFIX}phonhom remove @user",sty_warn); return

    remove_mode = parts[1].lower() in ("remove","rm")
    uid = None
    if remove_mode:
        uid=(message_object.mentions[0]["uid"] if message_object.mentions
             else parts[2] if len(parts)>2 and parts[2].isdigit() else None)
    else:
        uid=(message_object.mentions[0]["uid"] if message_object.mentions
             else parts[1] if parts[1].isdigit() else None)
    if not uid:
        _reply(client,message_object,thread_id,thread_type,
               f"WARNING\n    {PREFIX}phonhom @user",sty_warn); return

    try:
        if remove_mode:
            client.removeGroupAdmins([uid], thread_id)
            _mention_send(client, uid, thread_id, thread_type, "SUCCESS",
                          ["❌ Đã bị xóa khỏi phó nhóm!"], "#15A85F")
        else:
            client.addGroupAdmins([uid], thread_id)
            _mention_send(client, uid, thread_id, thread_type, "SUCCESS",
                          ["⭐ Đã được thêm làm phó nhóm!"], "#15A85F")
    except Exception as e:
        _reply(client,message_object,thread_id,thread_type,f"ERROR\n    {str(e)[:60]}",sty_err)

def Kryzis(): return {"phonhom": handle_phonhom}
