...
des = {"version":"1.0.0","credits":"kryzis X TXA","description":"Join nhóm qua link","power":"Admin"}

def handle_join(message, message_object, thread_id, thread_type, author_id, client):
    if not is_admin(author_id):
        _reply(client,message_object,thread_id,thread_type,"ERROR\n    Bạn không có quyền!",sty_err); return
    parts=message.split()
    if len(parts)<2 or not parts[1].startswith("https://zalo.me/g/"):
        _reply(client,message_object,thread_id,thread_type,
               f"WARNING\n    {PREFIX}join <link nhóm>",sty_warn); return
    link=parts[1]
    try:
        client.joinGroup(link)
        try:
            info=client.getIDsGroup(link)
            gname=info.get("name","?"); gid=info.get("groupId","?")
        except: gname="?"; gid="?"
        _mention_send(client, author_id, thread_id, thread_type, "JOIN THÀNH CÔNG ✅",
                      [f"🏷️ Nhóm: {gname}", f"🆔 ID: {gid}", f"🔗 Link: {link}"], "#15A85F")
