...
des = {"version":"1.0.0","credits":"kryzis X TXA","description":"Chuyển quyền chủ nhóm","power":"Admin"}

def handle_transferkey(message, message_object, thread_id, thread_type, author_id, client):
    ...
    parts=message.split()
    uid=(message_object.mentions[0]["uid"] if message_object.mentions
         else parts[1] if len(parts)>1 and parts[1].isdigit() else None)
    if not uid:
        _reply(client,message_object,thread_id,thread_type,
               f"WARNING\n    {PREFIX}transferkey @user",sty_warn); return

    try:
        gd = client.fetchGroupInfo(thread_id).gridInfoMap.get(str(thread_id),{})
        owner=str(gd.get("creatorId",""))
        bot=str(client.uid)
        if bot!=owner:
            _reply(client,message_object,thread_id,thread_type,"ERROR\n    Bot không phải chủ nhóm!",sty_err); return
        if uid==bot:
            _reply(client,message_object,thread_id,thread_type,"ERROR\n    Không thể chuyển cho bot!",sty_err); return

        confirm = len(parts)>2 and parts[-1].lower()=="confirm"
        if not confirm:
            name=_name(client,uid)
            _reply(client,message_object,thread_id,thread_type,
                   f"WARNING\n    Sắp chuyển quyền cho {name}\n    Thêm 'confirm' để xác nhận!",sty_warn); return

        client.changeGroupOwner(uid, thread_id)
        _mention_send(client, uid, thread_id, thread_type, "SUCCESS",
                      ["👑 Đã nhận quyền chủ nhóm!"], "#15A85F")
