...
def StartQuickCloudflared(*args, **kwargs):
    ...
    return None
def _orig_StartQuickCloudflared(*args, **kwargs):
    logger.base("⚠️ Cloudflared tunnel skipped - running locally only")
    return None

def _original_StartQuickCloudflared(port, cloudflaredPath="cloudflared"):
    cmd = [cloudflaredPath, "tunnel", "--url", f"http://127.0.0.1:{port}", "--loglevel", "info"]
    ...
    def Reader():
        ...
        try:
            for line in iter(p.stdout.readline, ""):
                out.append(line)
                urls = tryUrlPattern.findall(line)
                if not urls:
                    continue
                for raw in urls:
                    url = CleanUrl(raw)
                    ...
                    if "127.0.0.1" in lower or "localhost" in lower:
                        continue
                    if "trycloudflare.com" not in lower:
                        continue
                    TunnelState.AppServerUrl = url
            ...
            if not found:
                code = p.poll()
                if code is not None:
                    TunnelState.TunnelError = "".join(out[-120:]).strip()
                    TunnelState.TunnelReady.set()
...
def WaitTunnel(*args, **kwargs):
    ...
    return None
def _orig_WaitTunnel(timeout=300):
    ev = getattr(TunnelState, "TunnelReady", None)
    if not isinstance(ev, threading.Event):
        ev = threading.Event()
        TunnelState.TunnelReady = ev

    if not ev.wait(timeout):
        raise TimeoutError("Tunnel init timeout")

    url = getattr(TunnelState, "AppServerUrl", None)
    if url:
        return url

    err = (getattr(TunnelState, "TunnelError", "") or "").strip()
    if err:
        raise RuntimeError(err)

    p = getattr(TunnelState, "TunnelProc", None)
    code = p.poll() if p else None
    raise RuntimeError(f"Tunnel failed (code={code})")