...
from .worker.event import GroupEventType
...
def getClientMessageType(msgType):
	...
	if (msgType == "chat.sticker"): return 10
	...
	if (msgType == "chat.webcontent"): return 52
	if (msgType == "chat.webcontent.v2"): return 61
	return 1
...
def zalo_encode(params, key):
	try:
		...
		raise exception.EncodePayloadError(f"Unable to encode payload! Error: {e}")
...
def zalo_decode(params, key):
	try:
		...
		raise exception.DecodePayloadError(f"Unable to decode payload! Error: {e}")
