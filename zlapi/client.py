...
class ZaloAPI(object):
    ...
    def is_group_admin(self, group_id, user_id):
        """
        Kiểm tra xem user_id có phải là admin hoặc creator của nhóm group_id không.
        Sử dụng thông tin từ fetchGroupInfo.
        """
        try:
            ...
            # Kiểm tra nếu user_id là người tạo nhóm
            if user_id_str == str(creator_id):
                return True
            
            # Kiểm tra nếu user_id nằm trong danh sách admin
    ...
    # Thay thế hàm leaveReadOnlyGroups hiện tại bằng phiên bản này
    def leaveReadOnlyGroups(self, dry_run=False, whitelist_ids=None):
        """
        Quét và thoát các nhóm chỉ cho phép admin chat, nhưng bỏ qua nếu bot là admin/creator.
        """
        ...
        summary = {
            ...
            "failed_to_leave": [],
            "whitelisted_groups": [],
            "bot_is_admin_groups": [] # ** Thêm danh sách mới **
        }
        ...
        bot_uid = self.uid # Lấy ID của bot
        ...
        for group_id in group_ids:
            try:
                ...
                # 2. Nếu là nhóm chỉ đọc, kiểm tra quyền admin của bot
                if is_read_only:
                    # Lấy thông tin admin từ group_info đã fetch, không cần gọi API lần nữa
                    creator_id = group_info.get('creatorId')
                    admin_ids = group_info.get('adminIds', [])
                    ...
                    # 3. Nếu không thuộc 2 trường hợp trên, tiến hành thoát
                    ...
                    if not dry_run:
                        ...
                        summary["left_groups"].append({"id": group_id, "name": group_name})
                else: # Nhóm không phải chỉ đọc, giữ lại
                    summary["kept_groups"].append({"id": group_id, "name": group_name})
